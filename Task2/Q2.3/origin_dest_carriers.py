...
if __name__ == "__main__":
    ...
    def SaveResult(record):
        ...
        for i in res :
                ...
                dic_out = {"origin":origin , "dest":dest , "carrier":carrier , "avg_arr_delay":avg_delay}
        ...
        df.write.format("org.apache.spark.sql.cassandra").mode('overwrite').option('confirm.truncate',True).options(table="origin_dest", keyspace="cloudproject").save()


    st1 = ks.map(lambda x: x[1].split(',')).map(lambda x: ((x[3],x[4],x[1]),(float(x[8]),1)) if x[8]  else ('0',0)).filter(lambda x: x[0]!='0').updateStateByKey(updateFunction)
    st2 = st1.map(lambda (key , value): (key[0]+'_'+key[1] , [key[2] , value])).reduceByKey(gettop10)
    st2 = st1.foreachRDD(SaveResult)
